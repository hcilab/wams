...
class VideoPlayer {
  ...
  spawnPlayerWrapper(view) {
    ...
    this.app.spawn(
      html(`<div id="player-wrapper"></div>`, this.mainScreen.width, this.mainScreen.height, {
        x: 0,
        y: 0,
  ...
  spawnControls(options) {
    ...
    this.controls = this.app.spawn(
      html(markup, width, height, {
        x,
        y,
        width,
        height,
  ...
  handleDisconnect() {
    this.controls.moveTo(this.mainScreen.width / 2 - this.controls.width / 2, (this.mainScreen.height * 4) / 5);
  }
}
